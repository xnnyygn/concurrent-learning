...
public class UnfairTimedLock4 implements Lock {
    ...
    private static class ConditionImpl implements Condition {
        final LinkedList<CWaitingThread> waitingThreads = new LinkedList<>();
        final Lock lock;
        ...
        public void await() throws InterruptedException {
            waitingThreads.addLast(new CWaitingThread(Thread.currentThread()));
            lock.unlock();
            LockSupport.park(this);
        ...
        public boolean await(long time, @Nonnull TimeUnit unit) throws InterruptedException {
            CWaitingThread t = new CWaitingThread(Thread.currentThread());
            waitingThreads.addLast(t);
            lock.unlock();
            LockSupport.parkNanos(this, unit.toNanos(time));
            boolean signaled = (t.status.get() == CWaitingThread.STATUS_SIGNALED || !t.abort());
            lock.lock();
            return signaled;
        }
        ...
        @Override
        public void signal() {
            CWaitingThread t = waitingThreads.removeFirst();
            if (t != null) {
                t.wakeUp();
            }
        }
        ...
        public void signalAll() {
            for (CWaitingThread t : waitingThreads) {
                t.wakeUp();
            }
    ...
    private static class CWaitingThread {
        static final int STATUS_NORMAL = 0;
        ...
        CWaitingThread(@Nonnull Thread thread) {
            this.thread = new AtomicReference<>(thread);
        }
